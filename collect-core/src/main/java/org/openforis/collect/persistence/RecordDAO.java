...
import org.openforis.collect.persistence.jooq.RecordSummaryLoader;
...
public class RecordDAO extends CollectDAO {
	...
	public List<RecordSummary> loadRecordSummaries(EntityDefinition rootEntityDefinition, int offset, int maxNumberOfRecords, String orderByFieldName, String filter) {
		Factory jf = getJooqFactory();
		RecordSummaryLoader recordSummaryLoader = new RecordSummaryLoader(jf);
		List<RecordSummary> result = recordSummaryLoader.load(rootEntityDefinition, offset, maxNumberOfRecords, orderByFieldName, filter);
		return result;
	}
