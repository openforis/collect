...
class SurveySelect extends Component {
  ...
  handleSurveySelect(surveyId) {
    ...
    if (!selectedSurvey || selectedSurvey.id !== surveyId) {
      ...
      if (survey) {
        this.props.dispatch(selectActiveSurvey(surveyId))
      }
    }
  }
  ...
  render() {
    ...
    if (isEmpty) {
      ...
      const selectedSurveyName = selectedSurvey
        ? selectedSurvey.name
        : L.l('survey.selectPublishedSurveyDropdownHeading')
      const dropdownItems = publishedSurveys.map((s) => (
        <DropdownItem key={s.id} className="survey-item" onClick={this.handleSurveySelect.bind(this, s.id)}>
          ...
          <div className="survey-project-name">{s.projectName}</div>
        </DropdownItem>
      ))

      return (
        <UncontrolledDropdown className="survey-selector">
          <DropdownToggle className="survey-dropdown-toggle" caret>
            {selectedSurveyName}
          </DropdownToggle>
...
const mapStateToProps = (state) => {
  ...
  const { isFetching: isFetchingSummaries, lastUpdated: lastSummariesUpdate, items: summaries } = surveysList || {
    isFetching: true,
    items: [],
  }
