...
import { getRecordDataTableState, getRecordDataTableCurrentPage, getRecordDataTableOwnerIds,
    getRecordDataTableKeyValues, getRecordDataTableRecords, getRecordDataTableRecordsPerPage, getRecordDataTableShowOnlyMyOwnRecords,
    getRecordDataTableSortFields, getRecordDataTableSummaryValues } from "./state";
...
export const reloadRecordSummaries = () => dispatch => {
    dispatch(changeRecordSummariesPage(1))
}

const fetchRecordSummaries = () => (dispatch, getState) => {
    const state = getState()
    const survey = state.preferredSurvey.survey
    ...
    const filter = {
        keyValues: getRecordDataTableKeyValues(recordDataTableState),
        summaryValues: getRecordDataTableSummaryValues(recordDataTableState),
        ownerIds: onlyMyOwnRecords ? [userId] : getRecordDataTableOwnerIds(recordDataTableState)
    }
