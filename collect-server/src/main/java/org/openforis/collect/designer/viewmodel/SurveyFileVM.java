...
public class SurveyFileVM extends SurveyObjectBaseVM<SurveyFile> {
	...
	public void init(@ContextParam(ContextType.BINDER) Binder binder,
			@ExecutionArgParam("surveyFile") SurveyFile surveyFile, @ExecutionArgParam("newItem") Boolean newItem) {
		super.init();
			...
			if (newItem && (uploadedFiles == null || uploadedFiles.isEmpty())) {
				...
				if (uploadedFiles == null || validateFileContent(binder)) {
					...
					if (!wasNewItem) {
						...
						File uploadedFile = uploadedFiles != null && !uploadedFiles.isEmpty() ? uploadedFiles.get(0)
								: null;
				...
				if (!headersValidationResult.isSuccessful()) {
					...
					case INVALID_NUMBER_OF_PLOTS_WARNING:
						MessageUtil.showWarning("survey.file.error.warning_csv_size",
								CollectEarthGridTemplateGenerator.CSV_LENGTH_WARNING + "");
						break;
					case INVALID_NUMBER_OF_PLOTS_TOO_LARGE:
						MessageUtil.showWarning("survey.file.error.error_csv_size",
								CollectEarthGridTemplateGenerator.CSV_LENGTH_ERROR + "");
	...
	public void downloadExampleFile(@BindingParam("fileType") String fileType) throws IOException {
		...
		case COLLECT_EARTH_GRID:
			...
			String fileName = String.format("%s_grid_template_%s.csv", survey.getName(),
					Dates.formatDateTime(new Date()));
