...
public class NodeUpdateRequestSetProxy implements Proxy {
	...
	public NodeUpdateRequestSet toNodeUpdateRequestSet(CodeListManager codeListManager, RecordSessionManager sessionManager,
		...
		for (NodeUpdateRequestProxy<?> proxy : requests) {
			...
			if ( proxy instanceof BaseAttributeUpdateRequestProxy<?> ) {
				converted = ((BaseAttributeUpdateRequestProxy<?>) proxy).toAttributeUpdateRequest(codeListManager, sessionManager, record);
			} else {
